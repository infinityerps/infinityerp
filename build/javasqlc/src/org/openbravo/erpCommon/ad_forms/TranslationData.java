...
static Logger log4j = LogManager.getLogger();
  ...
  public static TranslationData[] select(ConnectionProvider connectionProvider, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    ResultSet result;
    Vector<TranslationData> vector = new Vector<TranslationData>(0);
  ...
  public static TranslationData[] trlColumns(ConnectionProvider connectionProvider, String tableName, String isReducedVersion)    throws ServletException {
    return trlColumns(connectionProvider, tableName, isReducedVersion, 0, 0);
  }

  public static TranslationData[] trlColumns(ConnectionProvider connectionProvider, String tableName, String isReducedVersion, int firstRegister, int numberRegisters)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      ...
      "            and 1=1";
    strSql = strSql + ((isReducedVersion.equals("isReducedVersion"))?" and upper(ColumnName) not like '%HELP%' and upper(ColumnName) not like '%DESCRIPTION%' and upper(ColumnName) not like '%MSGTIP%' ":"");
    strSql = strSql + 
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      QueryTimeOutUtil.getInstance().setQueryTimeOut(st, SessionInfo.getQueryProfile());
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, tableName);
      if (isReducedVersion != null && !(isReducedVersion.equals(""))) {
        }
