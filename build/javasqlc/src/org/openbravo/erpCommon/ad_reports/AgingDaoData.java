...
static Logger log4j = LogManager.getLogger();
  ...
  public static AgingDaoData selectCredit(ConnectionProvider connectionProvider, String currencyId, String org, String paidStatus, String asOfDate, String recOrPay, String cbPartnerId, String excludeReverseds, String pgLimit, String oraLimit1)    throws ServletException {
    AgingDaoData instance = new AgingDaoData();
    ...
    strSql = strSql + ((excludeReverseds.equals("excludeReverseds"))?"  AND (fp2.fin_rev_payment_id is null AND NOT EXISTS (SELECT 1 FROM fin_payment WHERE fin_rev_payment_id = fp2.fin_payment_id)) ":"");
    strSql = strSql + 
      "                       AND fpsd.isactive = 'Y'" +
      "                       AND fpsd.iscanceled = 'N'" +
      "                       AND (fpsd.fin_payment_schedule_invoice IS NULL" +
      ...
      "							       AND fp3.isreceipt = ?" +
      "							       AND 3=3";
    strSql = strSql + ((cbPartnerId==null || cbPartnerId.equals(""))?"":"  AND fp3.C_BPARTNER_ID IN" + cbPartnerId);
    strSql = strSql + ((excludeReverseds.equals("excludeReverseds"))?"  AND (fp3.fin_rev_payment_id is null AND NOT EXISTS (SELECT 1 FROM fin_payment WHERE fin_rev_payment_id = fp3.fin_payment_id)) ":"");
    strSql = strSql + 
    ...
    st = conn.prepareStatement(strSql);
      ...
      if (excludeReverseds != null && !(excludeReverseds.equals(""))) {
        }
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, asOfDate);
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, asOfDate);
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, recOrPay);
