...
static Logger log4j = LogManager.getLogger();
  ...
  public static String getServiceTimeoutThreshold(Connection conn, ConnectionProvider connectionProvider, String service)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
    ...
    st = connectionProvider.getPreparedStatement(conn, strSql);
      ...
      result = st.executeQuery();
      if(result.next()) {
        strReturn = UtilSql.getValue(result, "timeout_threshold");
      }
      result.close();
    } catch(SQLException e){
      if (log4j.isDebugEnabled()) {
